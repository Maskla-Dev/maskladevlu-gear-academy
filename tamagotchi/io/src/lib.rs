...
use gstd::{prelude::*, Decode, Encode, TypeInfo, Debug, ActorId, debug};
...
impl TamagotchiState{
    ...
    pub fn play(&mut self, current_block_height: u64){
        ...
        self.entertained += FILL_PER_ENTERTAINMENT - (BOREDOM_PER_BLOCK * (current_block_height - self.entertained_block));
    ...
    pub fn sleep(&mut self, current_block_height: u64){
        ...
        self.rested += FILL_PER_SLEEP - (ENERGY_PER_BLOCK * (current_block_height - self.rested_block));
        TamagotchiState::verify_limit(&mut self.rested);
    }
